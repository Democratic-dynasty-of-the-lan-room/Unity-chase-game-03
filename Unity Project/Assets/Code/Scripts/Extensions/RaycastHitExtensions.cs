...
public static class RaycastHitExtensions
{
    ...
    public static Vector3 GetCorrectNormalForSphere(this RaycastHit hit, Vector3 dir)
    {
        if (hit.collider != null)
        {
            ...
            {
                RaycastHit result;
                if (hit.collider.Raycast(new Ray(hit.point - dir * 0.01f, dir), out result, 0.011f))
                {
        ...
        // Return a default normal or handle the case where hit.collider is null
        return Vector3.up; // Adjust as needed
    }
}